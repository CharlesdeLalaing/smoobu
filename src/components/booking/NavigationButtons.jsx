...
export const NavigationButtons = ({
  ...
  const prevButtonStyles = `${baseButtonStyles} text-[#668E73] border border-[#668E73]
    ${(loading || disabled || !isSelectedRoomAvailable()) ? 'opacity-50 cursor-not-allowed' : 'hover:bg-[#668E73] hover:text-white'}`;
  const nextButtonStyles = `${baseButtonStyles} text-white bg-[#668E73] ml-auto
    ${(!isStepValid || loading || disabled || !isSelectedRoomAvailable()) ? 'opacity-50 cursor-not-allowed' : 'hover:bg-opacity-90'}`;
    ...
    <div className="flex justify-between">
      {currentStep > 1 && (
        <button
          type="button"
          onClick={prevStep}
          disabled={loading || disabled || !isSelectedRoomAvailable()}
      ...
      <button
        ...
        disabled={!isStepValid || loading || disabled || !isSelectedRoomAvailable()}

...
app.get("/api/backup-database", async (req, res) => {
  try {
    ...
    // Backup coupons collection
    try {
      const couponsSnapshot = await db.collection("coupons").get();
      backupData.coupons = {};
      couponsSnapshot.forEach((doc) => {
        backupData.coupons[doc.id] = doc.data();
      });
      console.log(`✅ Backed up ${couponsSnapshot.size} coupons`);
    } catch (couponsError) {
      console.log("⚠️ Coupons collection not found or empty");
      backupData.coupons = {};
    }
    ...
    res.json({
      success: true,
      message: "Complete database backup completed",
      timestamp: timestamp,
      filename: backupFilename,
      location: `${process.cwd()}/${backupFilename}`,
      collections: {
        bookings: bookingsSnapshot.size,
        coupons: backupData.coupons ? Object.keys(backupData.coupons).length : 0,
      },
      backup: backupData,
    });
  } catch (error) {
